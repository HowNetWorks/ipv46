...
const parseIP = require("../src/ipv46").default;
...
describe("IPv4", () => {
  ...
  describe("parser", () => {
    it("requires numbers and .", () => {
      expect(parseIP("x.y.z.0")).to.be.null;
    });
    it("disallows leading zeroes", () => {
      expect(parseIP("01.02.03.04")).to.be.null;
      expect(parseIP("001.002.003.004")).to.be.null;
    });
    it("disallows numbers larger than 255", () => {
      expect(parseIP("0.0.0.256")).to.be.null;
    });
    it("disallows numbers less than 0", () => {
      expect(parseIP("0.0.0.-1")).to.be.null;
    });
    it("disallows too many octets", () => {
      expect(parseIP("1.2.3.4.5")).to.be.null;
    });
    it("disallows too few octets", () => {
      expect(parseIP("1.2.3")).to.be.null;
    });
    it("disallows leading whitespace", () => {
      expect(parseIP(" 0.0.0.0")).to.be.null;
    });
  ...
  describe("formatting", () => {
    it("produces the parsed value", () => {
      expect(String(parseIP("127.0.0.1"))).to.equal("127.0.0.1");
    });
  });
});
...
describe("IPv6", () => {
  ...
  describe("parser", () => {
    it("disallows non-hexadecimal characters", () => {
      expect(parseIP("x:y:z:0:1:2:3:4")).to.be.null;
    });
    ...
    it("disallows leading whitespace", () => {
      expect(parseIP(" 1:2:3:4:5:6:7:8")).to.be.null;
    });
    it("disallows trailing whitespace", () => {
      expect(parseIP("1:2:3:4:5:6:7:8 ")).to.be.null;
    });
    ...
    it("allows max. 8 words", () => {
      expect(parseIP("0:1:2:3:4:5:6:7:8")).to.be.null;
    });
    it("requires 8 words when there is no ::", () => {
      expect(parseIP("1:2:3:4:5:6:7")).to.be.null;
    });
    it("counts :: as at least one word", () => {
      expect(parseIP("1:2:3:4::5:6:7:8")).to.be.null;
    });
    it("allows only one ::", () => {
      expect(parseIP("1::4::8")).to.be.null;
    });
    ...
    it("allows max. 4 characters per word", () => {
      expect(parseIP("00000:1:2:3:4:5:6:7")).to.be.null;
      expect(parseIP("0:00001:2:3:4:5:6:7")).to.be.null;
    });
    ...
    it("requires embedded IPv4 addresses to be well-formed", () => {
      expect(parseIP("2001:db8::192.0.2.256")).to.be.null;
      expect(parseIP("2001:db8::192.0.2.1.0")).to.be.null;
      expect(parseIP("2001:db8::192.0.2")).to.be.null;
      expect(parseIP("2001:db8::192.0")).to.be.null;
      expect(parseIP("2001:db8::192.0.2.")).to.be.null;
      expect(parseIP("2001:db8::.0.2.1")).to.be.null;
    });
    it("counts embedded IPv4 address as 2 words", () => {
      expect(parseIP("1:2:3:4:5:6:7:8:192.0.2.1")).to.be.null;
      expect(parseIP("1:2:3:4:5:6:7:192.0.2.1")).to.be.null;
    });
    it("disallows non-trailing IPv4 addresses", () => {
      expect(parseIP("192.0.2.1::")).to.be.null;
      expect(parseIP("0:192.0.2.1::")).to.be.null;
      expect(parseIP("0:0:192.0.2.1::")).to.be.null;
      expect(parseIP("::192.0.2.1:0:0:0")).to.be.null;
      expect(parseIP("::192.0.2.1:0:0")).to.be.null;
      expect(parseIP("::192.0.2.1:0")).to.be.null;
    });
  });
  ...
  describe("formatting", () => {
    it("doesn't output leading zeroes", () => {
      expect(
        String(parseIP("0001:0002:0003:0004:0005:0006:0007:0008"))
      ).to.equal("1:2:3:4:5:6:7:8");
    });
    it("doesn't output :: when there are no two or more consecutive zero words", () => {
      expect(String(parseIP("1:2:3:0:5:6:7:8"))).to.equal("1:2:3:0:5:6:7:8");
    });
    it("collapses 2-8 zeroes", () => {
      expect(String(parseIP("1:2:3:0:0:6:7:8"))).to.equal("1:2:3::6:7:8");
      expect(String(parseIP("1:2:3:0:0:0:7:8"))).to.equal("1:2:3::7:8");
      expect(String(parseIP("1:2:0:0:0:0:7:8"))).to.equal("1:2::7:8");
      expect(String(parseIP("1:2:0:0:0:0:0:8"))).to.equal("1:2::8");
      expect(String(parseIP("1:0:0:0:0:0:0:8"))).to.equal("1::8");
      expect(String(parseIP("0:0:0:0:0:0:0:8"))).to.equal("::8");
      expect(String(parseIP("0:0:0:0:0:0:0:0"))).to.equal("::");
    });
    it("collapses zeroes even from the beginning", () => {
      expect(String(parseIP("0:0:3:4:5:6:7:8"))).to.equal("::3:4:5:6:7:8");
    });
    it("collapses zeroes even from the end", () => {
      expect(String(parseIP("1:2:3:4:5:6:0:0"))).to.equal("1:2:3:4:5:6::");
    });
    it("collapses the longest run of zeroes", () => {
      expect(String(parseIP("1:0:0:4:5:0:0:0"))).to.equal("1:0:0:4:5::");
    });
    it("collapses the leftmost longest run of zeroes", () => {
      expect(String(parseIP("1:0:0:0:5:0:0:0"))).to.equal("1::5:0:0:0");
    });
    it("always outputs lower-case", () => {
      expect(String(parseIP("abcd:ef00::"))).to.equal("abcd:ef00::");
      expect(String(parseIP("ABCD:EF00::"))).to.equal("abcd:ef00::");
    });
    it("doesn't output embedded IPv4 addresses", () => {
      expect(String(parseIP("::192.0.2.1"))).to.equal("::c000:201");
      expect(String(parseIP("::ffff:192.0.2.1"))).to.equal("::ffff:c000:201");
      expect(String(parseIP("2001:db8::192.0.2.1"))).to.equal(
        "2001:db8::c000:201"
      );
    });
  });
...
describe("parsing", () => {
  ...
  it("disallows values with no . or :", () => {
    expect(parseIP("Hello, World!")).to.be.null;
  });
});
